...
{
    public partial class SearchingAlgorithmVisualiser : Form
    {
        SearchingAlgorithm algorithm;
        int currentStep;
        public SearchingAlgorithmVisualiser()
        {
            InitializeComponent();
            algorithm = new LinearSearch();
            currentStep = 0;
            algorithm.Search(new int[] { 5, 4, 3, 2, 1 }, 3);
        ...
        private void UpdateUI(SearchingAlgorithmState currentState)
        {
            ...
            lblTarget.Text = "Target: " + currentState.Target.ToString();
